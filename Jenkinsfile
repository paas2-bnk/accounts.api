pipeline {   
    ...
    stages {
        ...
        stage('docker build') {  
            steps {              
                script {
                    sh("/usr/local/bin/run-kaniko-job.sh --dockerfile Dockerfile --context https://github.com/mc-istio/accounts-api.git --destination kadirzade/mc-istio-accounts-api:${IMAGE_TAG}")  
